rm(list=ls())
setwd("~/MCMCHandbook/PrecipitationExample/dataProcessing/")
if(FALSE){
...
load("../samples/water_vapor_20230622.RData")
...
png(filename = "TotalPrecipitableWater.png", 
...
colA<-plotColorRF(dat=useObs , rangeDat = useObs,
                  ...
                  label = "Total Precipitable Water" , length.out = 1000,cex=0.5 , cex.main=1.5)
...
text(x=1.6, y = seq(0,1,l=5), 
     labels = round(seq(0,5,l=5),2), cex = 1.5)
