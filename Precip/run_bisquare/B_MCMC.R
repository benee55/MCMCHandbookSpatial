...
library(nimble);
setwd("~/Dropbox/GMU_BenLee/HandbookMCMC/Precip/run_bisquare/")
####################################
...
source("../../stanCode/batchmeans.R")# batchmeans source
...
save(summaryMat,chain_output,ptFinal,file="../output/mcmcPrecip.RData")
...
for(i in 1:50){
  ...
  summaryMat[[1]]<-round(summaryFunction(chain_output,
                                         totTime=ptFinal[3], bmseThresh = 0.01),3)
  save(summaryMat,chain_output,ptFinal,file="/scratch/slee287/precip/bisquare/mcmcPrecip.RData")
}
